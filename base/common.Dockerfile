...
RUN set -eux \
  ...
  ; curl --retry 3 -fsSL ${btm_url} | tar zxf - -C /usr/local/bin btm \
  \
  ; bdwh_ver=$(curl --retry 3 -fsSL https://api.github.com/repos/imsnif/bandwhich/releases/latest | jq -r '.tag_name') \
  ...
  ; curl --retry 3 -fsSL ${bdwh_url} | tar zxf - -C /usr/local/bin \
  \
